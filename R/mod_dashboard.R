...
mod_dashboard_server <- function(input, output, session, user_rv) {
  ...
  observeEvent(input$btn_relatorio, {
    ...
    rmd_content <- c(
      ...
      "",
      "<div class='section'>",
      "  <h2>Exemplo de performance individual (dados fictícios)</h2>",
      "  <p class='muted'>Exemplo ilustrativo por atleta: minutos, participação em gols e criação de chances.</p>",
      "",
      "```{r echo=FALSE}",
      "atletas <- data.frame(",
      "  Jogador = c('João Silva','Pedro Santos','Lucas Lima','Rafael Souza','Mateus Costa'),",
      "  Posição = c('ATA','MEI','VOL','ZAG','LAT'),",
      "  Minutos = c(270, 245, 300, 315, 280),",
      "  Gols = c(3, 1, 0, 1, 0),",
      "  Assistências = c(1, 2, 1, 0, 1),",
      "  `xG (90)` = c(0.58, 0.22, 0.08, 0.10, 0.05),",
      "  `xA (90)` = c(0.18, 0.31, 0.14, 0.05, 0.20),",
      "  `Passes-chave` = c(6, 9, 5, 2, 7)",
      ")",
      "knitr::kable(atletas, format = 'html', table.attr = 'class=\"table\"')",
      "```",
      "",
      "</div>",
      "",
